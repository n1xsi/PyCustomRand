def true_round(number: int | float, length: int = 0) -> int | float:
    ...
    # Логика прохода по цифрам в обратном порядке:
    ## 1-ая часть range: проход по цифрам в обратном порядке, начиная с той, что после нужного знака округления
    ## 2-ая: проход до первой цифры после запятой включительно (или до конца числа, если length == 0)
