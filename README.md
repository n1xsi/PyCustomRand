...
Меня не устраивает, что модуль `random` в Python генерирует недостаточно случайные числа (_особенно при нескольких генерациях подряд_), а модуль `round` довольно "грубо" округляет числа (_`round(1.5)` = 2 и в то же время `round(2.5)` = 2_), поэтому я решил написать _свою_ библиотеку для генерации псевдослучайных чисел.
