...
Меня не устраивает, что модуль `random` в Python генерирует недостаточно случайные числа (*особенно при нескольких генерациях подряд*), а модуль `round` довольно "грубо" округляет числа (*`round(1.5)` = 2 и в то же время `round(2.5)` = 2*), поэтому я решил написать *свою* библиотеку для генерации псевдослучайных чисел.
...
- [ ] Добавление новых функций (seed и др.) и распределений.
- [ ] Написание тестов.
- [ ] Публикация в PyPI.
- [ ] Доработка README.
