...
> [!WARNING]
> _Библиотека находится на стадии разработки_
> 
...
Меня не устраивает, что модуль `random` в Python генерирует недостаточно случайные числа (_особенно при нескольких генерациях подряд_), а модуль `round` довольно "грубо" округляет числа (_`round(1.5)` = 2 и в то же время `round(2.5)` = 2 _), поэтому я решил написать _свою_ библиотеку для генерации псевдослучайных чисел.
...
- [ ] Добавление новых функций (seed) и распределений.
- [ ] Написание тестов.
- [ ] Публикация в PyPI.
- [ ] Доработка README.md
